...
Having used R for some years now, I've come across a few instances where R's **Inf** keyword can be used to yield a practical outcome. For those unfamiliar with R's **Inf** keyword, it is defined as postive or negative a number divided by zero, respectively yielding postive or negative **Inf**.
...
Great, so how we can make practical use of infinity in R code? Three applications I've found are in R's 

  1. *cut()* function for upper and lower limits.
  1. *ggplot2::annotate()* positioning text in ggplot at a corner or edge.
  1. *integrate()* function when setting the upper and lower integral bounds
...
#**Inf** in integrate function - upper and lower bounds#

```{r}
dn <- function(x){dnorm(x = x, mean = 0, sd = 1)}
integrate(dn, -Inf,  c(1,0))
```

#**Inf** in the ggplot2::annotate function - text positioning#  
